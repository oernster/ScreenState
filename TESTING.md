...
| Apply keeps the stacking order (FR-027) | With Windows Terminal behind Claude on the same display, both maximised and both in the profile, press Apply: Terminal should still be behind Claude when the restore ends. The rule is held by `TestPlacingKeepsTheStackingOrder`. |
| A sign-in puts back the recorded stacking order (FR-083) | Capture a new profile with Windows Terminal behind Claude on the same display, both maximised, then sign out and in without touching anything until the splash says ready: Claude should be in front of Terminal. Press a key while the splash still says "Please wait" on a second run: the order should be left as it came up and the report should say so (FR-087). Profiles captured before this change hold no ranks and keep whatever order the applications made. |
